...
export class GitLogService {
  ...
  static calculateMetrics(commits: GitCommit[]): ContributionMetrics {
    ...
    for (const commit of commits) {
      ...
      const existingMonth = contributor.monthlyActivity.find(
        (m) => m.month === monthKey,
      );
      if (existingMonth) {
    ...
    for (const monthKey of Object.keys(monthlyStats)) {
      ...
      if (monthData) {
        monthData.contributors = Array.from(contributorMap.values()).filter(
          (c) => c.monthlyActivity.some((m) => m.month === monthKey),
        ).length;
      }
    }

    const contributorActivity = Array.from(contributorMap.values()).sort(
    ...
    const activeContributors = contributorActivity.filter(
   ...
   */
  private static calculatePonyFactor(
